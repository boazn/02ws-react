...
<link rel="stylesheet" href="css/main.php?lang=<?=$lang_idx;?>&type=" type="text/css">
        ...
        <link rel="stylesheet" href="css/sunset.css" type="text/css">
        <? }?>
        ...
        <link rel="stylesheet" href="css/dust.css" type="text/css">
        <? }?>
        <? if (!$current->is_light()) { ?>
        <link rel="stylesheet" href="css/night.css" type="text/css">
            <? if ($current->get_pm10() > 250) { ?>
            <link rel="stylesheet" href="css/dust-night.css" type="text/css">
            <? }?>
        <? }?>
        <? if (isRaining()){ ?>
	<link rel="stylesheet" href="css/rain.css" type="text/css">
        ...
        <? } else {?>
        <link rel="stylesheet" href="css/snow_night.css" type="text/css">
        <? }?>
        <? }?>
	...
	<table <? if (isHeb()) echo "dir=\"rtl\""; ?> style="border-spacing:2px;padding:3px;width:100%">
        <? if  (count($forecastDaysDB) == 0) echo $frcstTable;
                    else 
                    {
                            ...
                            <td style="width:40px;text-align:center;font-weight:bold;line-height: 0.9em;"><?echo replaceDays($forecastDaysDB[$i]['day_name']." ")."<br />".$forecastDaysDB[$i]['date'];?></td>
                            <td style="text-align:center;direction:ltr"><?=c_or_f($forecastDaysDB[$i]['TempLow'])?></td>
                            <td style="text-align:center;direction:ltr"><?=c_or_f($forecastDaysDB[$i]['TempHigh'])?><a href="WhatToWear.php#<?=$prefTempHighCloth?>" rel="external" ><img style="vertical-align: middle" src="<? echo "images/clothes/".$forecastDaysDB[$i]['TempHighCloth']; ?>" width="25" height="20" title="<?=getClothTitle($forecastDaysDB[$i]['TempHighCloth'])?>" alt="<?=getClothTitle($forecastDaysDB[$i]['TempHighCloth'])?>" /></a></td>

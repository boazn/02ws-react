...
foreach($rawdata as $key) {
    if ($got < $wanted) {
        ...
        if (freq_check($timeA[1])) {
            ...
            debug_out("Yaxis = " . ret_value("solarrad") );
...
$graph->title->SetColor("azure4@0.3");
$graph->title->SetPos(0.53,0.94,"center","top");
...
//$graph->yaxis->SetLabelFormat('%001f');

// Print Wording on graphic

$txt2=new Text("W/M2");
$txt2->SetFont(FF_VERDANA, FS_BOLD,8);
$txt2->ParagraphAlign('left');
$txt2->SetPos(0.03,0.55,"left","center");
$txt2->SetColor("azure4@0.3");
$txt2->SetAngle(90);
$graph->AddText($txt2);

$chart_title = $RADIATION[$lang_idx]."/".$WIND[$lang_idx];

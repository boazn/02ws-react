...
foreach($rawdata as $key) {
    if ($got < $wanted) {
        ...
        if (freq_check($timeA[1])) {
            ...
            debug_out("Y2axis = " . ret_value("temp") );
...
$graph->yaxis->SetLabelFormat('%d ' . $SITE['windunit']);
...
$graph->y2axis->SetLabelFormat('%0.1f' . $SITE['tempunit']);
...
$lineplot->SetFillColor("azure4@0.5");
...
$chart_title = $WIND[$lang_idx]."/".$TEMP[$lang_idx];
...
$txtaa=new Text($chrs . "Temp");
$txtaa->SetFont(FF_VERDANA, FS_BOLD,9);
$txtaa->SetPos($width - 26,$height - 46,'center');
$txtaa->SetColor("azure4");
$graph->AddText($txtaa);


$txtab=new Text($chrs . "Wind");
$txtab->SetFont(FF_VERDANA, FS_BOLD,9);
$txtab->SetPos(22,$height - 46,'center');
$txtab->SetColor("red");
$graph->AddText($txtab);

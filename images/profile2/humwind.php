...
foreach($rawdata as $key) {
    ...
    if ($got < $wanted) {
        ...
        if (freq_check($timeA[1])) {
            ...
            debug_out("Y1axis = " . ret_value("hum") );
            debug_out("Y2axis = " . ret_value("winddir") );

            //$datetime = DateTime::createFromFormat("j/m/y H:i", ret_value("date ")." ".$timeA[0].":".$timeA[1]);
	        ...
	        $got++;
                if ($got == 1){
                    $least_date_title = ret_value("date");
                }
                if ($got == $wanted){
                    $date_title  = ret_value("date")." - ".$least_date_title;
                }
...
$graph->ygrid->SetFill(true,'#ECECEC@0.5','#FFFFFF@0.8');
$graph->yaxis->HideTicks(false,false); 
$graph->ygrid->Show(true);

//y2-axis
$graph->y2axis->SetFont(FF_VERDANA,FS_NORMAL,8);
$graph->y2axis->SetColor($SITE['txtcolor']);
$graph->y2axis->HideTicks(true,true);
$graph->y2axis->SetTickLabels($SITE['compass']);
...
$chart_title = $HUMIDITY[$lang_idx]."/".$WIND_DIR[$lang_idx];
...
$txt3=new Text($date_title);
$txt3->SetFont(FF_VERDANA, FS_BOLD,8);
$txt3->ParagraphAlign('left');
$txt3->SetAlign('left');
$txt3->SetPos(220,0.015,"left","top");
...
$temp = $lang_idx == 1 ? utf8_strrev($HUMIDITY[$lang_idx]) : $HUMIDITY[$lang_idx];
$txtab=new Text($chrs . $temp);
$txtab->SetFont(FF_ARIAL, FS_BOLD,9);
$txtab->SetPos(26,$height - 220,'center');
$txtab->SetColor("blue");
$graph->AddText($txtab);

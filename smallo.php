...
<div id="nextdays" style="visibility:hidden">
    <div id="for_title" class="float">
        ...
        <a href="javascript:void(0)" id="" onclick="navMain('messages_box', $(this).parent().attr('id'))">
...
<div id="latestalert" style="visibility:hidden;min-height:10px">
</div>
...
/* exported loadCSS */
    ...
    function loadData(json)
    {
      ...
      cssstyle_str += ",mobile";
        ...
        $('#tempdivvalue').html('<div class="shade">' + ((json.jws.current.islight == 1) ? "" : "") + '</div>' + c_or_f(json.jws.current.temp, tempunit)+'<div class="param">'+tempunit+'</div>');
        $('#tempdivvalue').css('visibility', 'visible');
        if (json.jws.Messages.passedts < 7000){
            var lastindex = 150;
            containsanchor = latestalerttext.indexOf("<a");
            ...
            if ((latestalerttext.length > lastindex)&&!(containsanchor > 0))
                latestalerttext = "<a href=\"javascript:void(0)\" class=\"info\" >" + latestalerttext.substring(0, partialtext.lastIndexOf(" ")) + "..." + "<span class=\"info\"> " + latestalerttext + "</span></a>" + "</td>";
       ...
       if (typeof coldmeter_size == 'undefined') 
        ...
        if (json.jws.states.sigweather.length > 1)
            $('#what_is_h').html(json.jws.states.sigweather[0].sigtitle<? echo $lang_idx;?>+
                                 '&nbsp;<div class=\"invhigh\">&nbsp;' + 
                                  (json.jws.states.sigweather.length - 1) + 
                                  '&nbsp;</div>'
                                   + '<br/><div class="rainpercent">' + (json.jws.current.rainchance > 0 ? (json.jws.current.rainchance + '%') : '') + '</div>');
       ...
       forecastDays += "</tr>";
        for (i = 0; i< json.jws.forecastDays.length; i++){
            TempLowCloth = '<a href=\"javascript:void(0)\" class=\"info\" ><img style=\"vertical-align: middle\" src=\"'+json.jws.forecastDays[i].TempLowCloth+'\" width=\"17\" height=\"14\" title=\"'+json.jws.forecastDays[i].TempLowClothTitle<? echo $lang_idx;?>+'\" alt=\"\" /><span class=\"info\">'+json.jws.forecastDays[i].TempLowClothTitle<? echo $lang_idx;?>+'</span></a>';
            TempHighCloth = '<a href=\"javascript:void(0)\" class=\"info\" ><img style=\"vertical-align: middle\" src=\"'+json.jws.forecastDays[i].TempHighCloth+'\" width=\"17\" height=\"14\" title=\"'+json.jws.forecastDays[i].TempHighClothTitle<? echo $lang_idx;?>+'\" alt=\"\" /><span class=\"info\">'+json.jws.forecastDays[i].TempHighClothTitle<? echo $lang_idx;?>+'</span></a>';
            TempNightCloth = '<a href=\"javascript:void(0)\" class=\"info\" ><img style=\"vertical-align: middle\" src=\"'+json.jws.forecastDays[i].TempNightCloth+'\" width=\"17\" height=\"14\" title=\"'+json.jws.forecastDays[i].TempNightClothTitle<? echo $lang_idx;?>+'\" alt=\"\" /><span class=\"info\">'+json.jws.forecastDays[i].TempNightClothTitle<? echo $lang_idx;?>+'</span></a>';
            ...
            if (i==0)
              link_for_yest = "<a hrf=\"javascript:void(0)\" onclick=\"$('#yesterday_line').show();\"><img src=\"images/yesterday.png\" width=\"14\" height=\"14\" title=\"<?=$LAST_DAY[$lang_idx]?>\" /></a>&nbsp;&nbsp;";
                ...
                link_for_yest = "";
            forecastDays += "<td class=\"tsfh datef\">" + link_for_yest + json.jws.forecastDays[i].day_name<?=$lang_idx?> + "<br />" + json.jws.forecastDays[i].date + "</td>";
            forecastDays += "<td class=\"tsfh\" style=\"text-align:center;direction:ltr\">" + c_or_f(json.jws.forecastDays[i].TempLow, tempunit) + TempLowCloth + "</td>";
       ...
       forecastDays += "<tr><td style=\"text-align:center\">" + json.jws.desc.month_in_word<?=$lang_idx?> + " <?=$AVERAGE[$lang_idx]?></td><td class=\"tsfh\" style=\"text-align:center\">" + json.jws.thisMonth.lowtemp_av + "</td><td class=\"tsfh\" style=\"text-align:center\">" + json.jws.thisMonth.hightemp_av + "</td><td></td><td></td><td></td></tr>";
       forecastDays += "</table>";
   ...
   {
      navMain('forecastnextdays', 'fornextdays_title');
...
if (!isUserAdApproved)
	{
		$("#adunit1").css('visibility', 'visible');
		if (('#adunit3').length)
			 $('#adunit3').show();
		 $('#adunit2').show();
	}

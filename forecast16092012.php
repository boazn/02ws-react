...
$taf_server_file = "/data/forecasts/taf/stations/".$station_code.".TXT"; //OJAM.TXT LLBG.TXT
...
function checktofloatList ($licount)
{
	...
	function isLastTokenIsProb()
	{
        ...
        function updateForecastHour($currentPri, $title, $icon){
            ...
            foreach ($forecastHour as &$hour_f){
		     ...
		     if ((($icon != "")&&($currentPri > $hour_f['priority'] ))||($new_line&&($currentPri < $hour_f['priority'])))
			{
				$hour_f['icon'] = $icon;
				if ($_GET["debug"] >= 3){
					 echo " icon into ".$icon;
			 ...
			 if ((($last_priority == 20)&&(!$isProb))||($new_line&&($currentPri < $hour_f['priority'])))
			 {
	...
	function updateForecast($currentPri, $title, $pic)
	{
		 ...
		 if ($_GET["debug"] >= 3)
		{
		 ...
		 {
			 if (($priority == 0)||($priority >= 50) || ($currentPri >= 50))
			 {
		...
		if (stristr ($taf_tokens[$i], ":")) {
			...
			array_push($forecast_title, "<span>".$GENERALLY[$lang_idx]." "."</span>");
			 for ($t=$timetaf + 1; $t <= $timetaf+30 ; $t++)
			 {
				 ...
				 array_push($forecastHour, array('id' => $hourindex, 'time' => $time, 'currentDateTime' => $currentDateTime, 'temp' => "", 'icon' => "", 'title' => "", 'cloth' => "", 'priority' => 0));
				 $hourindex += 1;
...
?>
		...
		<div id="forecast24h">
				...
				<div class="slideshow" id="sigweather_cycle" style="position: relative">
					...
					<div class="sigweather inv_plain_3_zebra" id="first" style="height:100%" title="<? echo($FORECAST_TITLE[$lang_idx]." ".$FOR[$lang_idx]." 24 ".$HOURS[$lang_idx]);?>">
						...
						<div id="forecasthours" style="clear:both;width:100%;padding:0.5em" class="small">
						 <? 
						 foreach ($forecastHour as $hour_f){
						 if ($hour_f['time'] % 3 == 0)
						 {
						 echo "<ul class=\"float forecasttimebox\" >";
						 echo "<li class=\"inv_plain timefh\" style=\"text-align:center\">".$hour_f['time']."</li>";
						 echo "<li class=\"inv_plain_3_minus\" style=\"text-align:center\" id=\"tempfh".intval($hour_f['time'])."\">".$hour_f['temp']."</li>";
						 echo "<li class=\"inv_plain_3_minus small\" style=\"text-align:center\"><img src=\"images/icons/day/".$hour_f['icon']."\" height=\"30\" width=\"35\" alt=\"".$hour_f['icon']."\" /></li>";
						 echo "<li class=\"inv_plain_3_minus\">".$hour_f['title']."</li>";
						 echo "</ul>";
						 }
						 }
						 ?>
						...
						<div class="overlay">&nbsp;</div>
							<div class="title">
								<a href="#" class="hlink">
										...
										<span class="big"><? echo($FORECAST_TITLE[$lang_idx]." ".$FOR[$lang_idx]." 24 ".$HOURS[$lang_idx]);?></span>
										<br />
		...
		<div id="nextdays">
			<div id="for_title">
				<div id="for_title_title" class="slogan">
					<a href="<? echo get_query_edited_url(get_url(), 'section', 'forecast.php');?>" >
			...
			<div id="forecastnextdays" >
				...
				<? if  (count($forecastDaysDB) == 0) 
					...
					else 
					{
						...
						<td style="text-align:center"><img src="<? echo "images/icons/day/".$forecastDaysDB[$i]['icon']; ?>" width="60" height="48" alt="<?=$forecastDaysDB[$i]['date']?>" /></td>
				...
				<tr>
				<td>
					<a href="<? echo get_query_edited_url($url_cur, 'section', 'averages.php');?>" title="<? echo $AVERAGE[$lang_idx];?>">
				...
				<td>
					...
					<a href="<? echo get_query_edited_url(get_url(), 'section', 'dust.html');?>" title="<? echo($FORECAST_TITLE[$lang_idx]." ".$FOR[$lang_idx]." ".$DUST[$lang_idx]); ?>"><? echo($DUST[$lang_idx]); ?><?=get_arrow()?></a>&nbsp;&nbsp;
					...
					<div class="invfloat">
					<a href="legend.php" class="colorbox" title="legend"><?=$LEGEND[$lang_idx]?><?=get_arrow()?></a>&nbsp;&nbsp;
					</div>
				</td>
				</tr>
                                <tr>
				<td>
					<a href="<? echo get_query_edited_url($url_cur, 'section', 'reports.php');?>" title="<? echo $RECORDS[$lang_idx];?>">
						<? echo $monthInWord." ".$RECORDS[$lang_idx]; ?>
...
<!--
	...
	{
		if (ind == 0) return '<li><a href="#" title="<? echo($GIVEN[$lang_idx]." ".$AT[$lang_idx]." ".$timetaf.":00"." ");?>" ><? echo $FORECAST_TITLE[$lang_idx];?></a></li>';//id="first"
               else if (ind == 1) return '<li><a href="#" title=""><? echo $MESSAGES[$lang_idx];?></a></li>';
              else if (ind == 2) return '<li><a href="<?=$sig_url?>" title="<?=$CURRENT_SIG_WEATHER[$lang_idx]?>"><?=$sig_title?></a></li>';
	       else if (ind == 3) return '<li><a href="<?=$webcam_link?>" title="<? echo $PIC_DESC[$lang_idx];?>"><? echo $LIVE_PICTURE[$lang_idx];?></a></li>';
		  else if (ind == 4) return '<li><a href="<? echo "station.php?section=mainstory.php&amp;lang=".$lang_idx;?>" title="<? echo $current_story_title;?>"><? echo $current_story_title;?></a></li>';
		  else if (ind == 5) return '<li><a href="#" title="<? echo $PIC_OF_THE_DAY[$lang_idx]?>"><? echo $PIC_OF_THE_DAY[$lang_idx]?></a></li>';
		  else
			return "";

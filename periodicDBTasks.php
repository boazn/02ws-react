...
function updatePeriodicCachedVars(){
    ...
    $mem->set(YEST_MORNING_TEMP, $line["temp"]);
    if ($_GET['debug'] >= 1){
    ...
    $mem->set(YEST_NOON_TEMP, $line["temp"]);
    if ($_GET['debug'] >= 1)
    ...
    $result = mysqli_query($link, $query) ;
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
    $mem->set(TODAY_NIGHT_TEMP, $row["temp"]);
    if ($_GET['debug'] >= 1)
    ...
    $result = mysqli_query($link, $query) ;
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
    $mem->set(TODAY_MORNING_TEMP, $row["temp"]);
    if ($_GET['debug'] >= 1)
    ...
    $result = mysqli_query($link, $query) ;
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
    $mem->set(TODAY_NOON_TEMP, $row["temp"]);
    if ($_GET['debug'] >= 1)
    ...
    $result = mysqli_query($link, $query) ;
    $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
    $mem->set(SUNSHINE_HOURS, round($row["minutes"]/60, 1));
    if ($_GET['debug'] >= 1)
	echo( "<br/>stored ".round($row["minutes"]/60)." in SunshineHours");
    
    $query = "SELECT MAX(`Rain2`) dailyrain, Date FROM `archivelatest` group by Date Order by Date Desc";
    $last7daysDailyRain = array();
    $result = mysqli_query($link, $query) ;
    while ($line = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
        array_push($last7daysDailyRain, array("DailyRain" => $line["dailyrain"], "Date" => $line["Date"]));
    }    
    $mem->set(LAST_7DAYS_DAILY_RAIN, $last7daysDailyRain);
    return true;
}
db_init("", "");

...
class DB_Functions {
     ...
     */
    public function storeUser($name, $email, $apn_regid, $lang, $active, $active_shortterm, $active_tips) {
        ...
        $result = db_init("call SaveAPNUser ('$name', '$email', '$apn_regid', $lang, $active, $active_shortterm, $active_tips)", "");
        // check for successful store
       
        // get user details
        $id = $link->insert_id; // last inserted id
        //logger("New APN user updated:".$id." ".$name." ".$email." ".$apn_regid." ".$lang." active=".$active." active_rain_etc=".$active_shortterm." active_tips=".$active_tips);
        return $email;
...
if (isset($_POST["name"]) && isset($_POST["regId"])&& isset($_POST["lang"])&& isset($_POST["active"])) {
    ...
    $res = $db->storeUser($name, $email, $apn_regid, $lang, $active, $active_shortterm, $active_tips);
    echo ($res);
} else {

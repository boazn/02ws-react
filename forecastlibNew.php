...
$endDateTime = "";
$plusminus = 0;
list($fday, $fmonth, $fyear) = split('[/]', $dayF."/".$monthF."/".$yearF);
...
function updateForecast($currentPri, $title, $pic)
{
        ...
        if ($hour > 9 &&  (get_sunset_ut() - $current->get_current_time_ut() > 8600) && ($current->get_solarradiation() < 400))
        {
         ...
         if ($hour > 9 && (get_sunset_ut() - $current->get_current_time_ut() > 6000) && ($current->get_solarradiation() < 200))
         {
...
for ($i = 0; $i < count($taf_tokens); $i++)
{
        ...
        if ($i == 0)
        {
                ...
                apc_store("dayF", $dayF);
                apc_store("monthF", $monthF);
                apc_store("yearF", $yearF);
  }
                ...
                array_push($forecast_title, array("<span>".$GENERALLY[$EN]." "."</span>", "<span>".$GENERALLY[$HEB]." "."</span>"));
                 ...
                 for ($t=$timetaf + 1; $t <= $timetaf+30 ; $t++)
                 {
                  ...
                  //$clothHour = getCloth($tempHour);
                   array_push($forecastHour, array('id' => $hourindex, 'time' => $time, 'currentDateTime' => $currentDateTime, 'plusminus' => 0, 'change' => 0, 'temp' => $tempHour, 'wind' => 0, 'icon' => "", 'title' => array(), 'cloth' => $clothHour, 'priority' => 0));
                   $hourindex += 1;
        ...
        if (stristr ($taf_tokens[$i], "TSRA")) updateForecast(85, array($THUNDERSTORM[$EN].", ".$RAIN[$EN], $THUNDERSTORM[$HEB].", ".$RAIN[$HEB]), "TSRA.gif");
                ...
                (stristr ($taf_tokens[$i], "-RA"))) updateForecast(75, $LIGHT_RAIN, "lightrain.gif");
        else if (stristr ($taf_tokens[$i], "RA")) updateForecast(78, $RAIN, "rainy.gif");
      
        if (stristr ($taf_tokens[$i], "DZ")) updateForecast(68, $DRIZZLE, "showersl2.png");
        ...
        if (stristr ($taf_tokens[$i], "TEMPO"))    {
                ...
                else if (stristr ($forecast_title[count($forecast_title) - 1][$lang_idx], $PROB[$lang_idx]))
                {	

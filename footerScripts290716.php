...
function startup(lang, from, update)
     ...
     function getWindInfo(windspeed){
      ...
      console.log("position in custom tooltip");
       var position = $(this._chart.canvas)[0].getBoundingClientRect();
	   var tooltipwidth = 140;
	   var tooltipx = tooltip.x + (position.width/3);
	   var tooltipleft = position.left + tooltipx;
	   
		if (tooltipleft < -3*tooltipwidth)
			tooltipleft = tooltipleft + (5*tooltipwidth);
	    else if (tooltipleft < -2*tooltipwidth)
			tooltipleft = tooltipleft + (4*tooltipwidth);
		else if (tooltipleft < -tooltipwidth)
			tooltipleft = tooltipleft + (3*tooltipwidth);	
		else if (tooltipleft < -(tooltipwidth/3))
			tooltipleft = tooltipleft + (2*tooltipwidth);	
	    else if (tooltipleft < tooltipwidth)
		    tooltipleft = tooltipleft + tooltipwidth;
		if (tooltipleft > (position.width - (tooltipwidth/2)))
		 tooltipleft = tooltipleft - tooltipwidth;
    ...
    var size = 30;
    var barChartData = {
            labels: [<? $forecastHour = apc_fetch("forecasthour");
        ...
        ?>],
            datasets: [{
                    label: "<?= $TEMP[$lang_idx] ?>",
                    type: 'line',
                   data: [<? $max_temp = -10; $min_temp = 100;
        foreach ($forecastHour as $key => &$hour_f) {
            echo $hour_f['temp']; if ($hour_f['temp'] > $max_temp) {$max_temp = $hour_f['temp'];} if ($hour_f['temp'] < $min_temp) {$min_temp = $hour_f['temp'];}
            if ($key != $last_key) {
                echo ",";
            }
        }
        ?>],
                    ...
                    pointBorderColor: '#FF6550',
                    pointBackgroundColor: '#FF6550',
                    pointHoverBackgroundColor: '#FF6550',
                    pointHoverBorderColor: '#EC232F',
                    pointRadius:3,
                    pointHitRadius:9,
                    pointHoverRadius:9,
                    yAxisID: 'y-axis-1'
        ...
        ?>],
                    fill: false,
                    borderColor: 'rgba(236,147,47,0.4)',
                    backgroundColor: 'rgba(236,147,47,0.4)',
                    pointBorderColor: 'rgba(236,147,47,0.4)',
                    pointBackgroundColor: 'rgba(236,147,47,0.4)',
                    pointRadius:1,
                    pointHitRadius:9,
                    yAxisID: 'y-axis-2'
            } , {
		...
		$imgi = 1;
            ...
            Chart.helpers.extend(Chart.controllers.bar.prototype, {
                ...
                draw: function() {
                        ...
                        [
                            <? foreach ($sigforecastHour as $hour_f){
                                echo "{ x: ".(($hour_f['id']+1)-0.5).", y: ".{($max_temp + $min_temp)/2 + 2}."}";
                                if ($hour_f != end($sigforecastHour))
                                    echo ",";
                            }
                            ?>
				     ...
				     ctx.fillStyle = "#000";
                                    ctx.fillText(xlabel, p._model.x, (chart.chart.canvas.height - 100));
				}

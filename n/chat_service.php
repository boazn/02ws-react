...
function insertNewMessage ($name, $icon, $body, $category)
{
	   if (empty($_SESSION['loggedin'])||($_SESSION['loggedin']=="false"))
		...
		$body = nl2br($body);
		$query = "INSERT INTO chat (date_chat, last_date_chat, body, name, user_icon, category, sticky, IP) VALUES('{$now}', '{$now}', '{$body}', '{$name}', '$icon', '$category', '0', '{$_SERVER['REMOTE_ADDR']}');";
		//echo $query;
...
{
	$query = "select IP, name, body, date_chat  from chat WHERE (idx=$idx)";
	//echo $query;
	$result = db_init( $query);
...
function AddToMessage($name, $user_icon, $body, $idx, $mood)
{
	...
	$body = $prev_body." ".$clearboth.HIDDENSEPERATOR." <div class=\"float chataftersepreply ".$firstreplyline." \"><div class=\"float chatbodyreply ".$firstreplyline."\">".SEPERATOR."<div class=\"avatar ".$user_icon."\"></div><strong><span title=\"".base64_encode($_SERVER['REMOTE_ADDR'])."\" style=\"\">".$mood.$name."</span></strong>&nbsp;".$body."</div><div class=\"chatdatereply\">".replaceDays(date('D H:i', strtotime(SERVER_CLOCK_DIFF, time())))."</div></div>";
	checkSpam();
...
{
		$now = date('Y-m-d G:i:s');
		$query = "UPDATE chat SET Locked='{$lockValue}' WHERE (idx=$idx)";
		//echo $query;
		$result = db_init($query);
		if (!$result) {
			$message  = 'Invalid query: ' . mysql_error() . "\n";
...
{
    $timestamp_filter = mktime (0, 0, 0, substr($limitInput,1,2), substr($limitInput,0,1) ,substr($limitInput,3,4));
    $filter_is_on = true;
}
...
if ($_REQUEST['update'] == 1)
    $category = "";
getCurrentChat($searchname, $filter_is_on, $startLine, $limitLines, $timestamp_filter, $category);

                ...
                draw: function() {
                        ...
                        [
                            ...
                            foreach ($sigWindHour as $hour_f){
                                if (getCorF($hour_f['temp']) == getCorF($min_temp)) $y_image = getCorF($hour_f['temp']) + 5.2; else $y_image = getCorF($hour_f['temp']) - 1;
                    ...
                    /*animation: {
                        ...
                        onComplete: function() {
                            ...
                            xAxis.ticks.forEach(function(value, index) {
                                var xOffset = xAxis.getPixelForValue(undefined,index) + halfBarWidth;
                                var yOffset = 26;
...
function pressingDown(e) {
    ...
    function timerduring(){
        //disp.innerHTML = counter;
        fontsize = parseFloat($("#statuslinestart").css('font-size'));
        if (fontsize < 50)
            $("#statuslinestart").css('font-size', fontsize*1.01);
    }
    function timerdone(){
        //disp.innerHTML = "Press threshold reached!";
        counter = 0;
        if (current_tab == 'startinfo_container')
            navMain('currentinfo_container', 'now_title');
    }
    ...
    var item = document.querySelector("#logo");
    ...
    // event fires
    var pressHoldDuration = 25;
    
    function isSwipePermitted(swipedir, startX, startY, pageID)
    {
        return (startY > 380 && pageID == 5 && ((swipedir.toLowerCase() == "right") || (swipedir.toLowerCase() == "left")));
    }
    
    function handleswipefun(swipedir, startX, startY){
        prevPageID = pageID;
        //console.log(startX + ' ' + startY + ' ' + pageID);
        if (isSwipePermitted(swipedir, startX, startY, pageID))//no swipe in 24H graph
           swipedir = 'none';
        if (swipedir.toLowerCase() == "right"){
            if (pageID == 2)
                  pageID  = 1;
               else if (pageID == 3)
                  pageID  = 2;
        ...
        if (swipedir.toLowerCase() == "none")
            return;
              
        switch (pageID) {
            case 1:
               navMain('forecastnextdays', 'forecastnextdays', swipedir.toLowerCase());
               
               break;
            case 2:
               navMain('startinfo_container', 'now', swipedir.toLowerCase());
               break;
            case 3:
               navMain('messages_box', 'alerts', swipedir.toLowerCase());
               break ;
            case 4:
               navMain('forecastnextdays', 'forecastnextdays', swipedir.toLowerCase());
               break;
           case 5:
               navMain('currentinfo_container', 'now', swipedir.toLowerCase());
               break;
           case 6:
              navMain('messages_box', 'alerts', swipedir.toLowerCase());
              break;
           case 7:
               ...
               break;
           case 8:
               navMain('forcast_hours', 'now', swipedir.toLowerCase());
               break;
           case 9:
              navMain('messages_box', 'alerts', swipedir.toLowerCase());
              break;
...
rememberCookie ('sessions', sessions, 30);
...
var el = document.getElementById('main_cellphone_container');
var disp = document.getElementById('tempdivvaluestart');
swipedetect(el, function(swipedir, startX, startY){
    //disp.innerHTML = 'Swiped <span style="color:yellow;margin: 0 5px;">' + swipedir +'</span> ';
    //disp.style.visibility = "visible";
    handleswipefun(swipedir, startX, startY)
});

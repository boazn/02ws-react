...
function get_user(){
    ...
    //logger("get_user: email=".$_GET['email']." sessionEmail=".$_SESSION['email']." rememberme=".$rememberme, 0, "auth", "db", "get_user");
    $userJSON = "{\"user\":";
     if (!empty($_SESSION['email']) && is_valid_email($_SESSION['email'])) {
        ...
        */
        ...
        logger( $_SESSION['email']." authenticated with Session: "."\"display\":"."\"".$line['display_name']."\""." \"nicename\":"."\"".$line['user_nicename']."\""." \"priority\":".$line['priority']." \"admin\":".$line['admin'], 0, "auth", "db", "get_user");
        $_SESSION['loggedin'] = "true";
    }
    else if (!empty($_GET['email'])){
        $line = get_user_from_email($_GET['email']);
        //logger( $_GET['email']." authenticated with _GET: "."\"display\":"."\"".$line['display_name']."\""." \"nicename\":"."\"".$line['user_nicename']."\""." \"priority\":".$line['priority']." \"admin\":".$line['admin'], 0, "auth", "db", "get_user");
        $_SESSION['loggedin'] = "true";
        ...
        if (!empty($rememberme)) {
            ...
            if (!empty($line[0])&&($line[0]==1))
            {
                ...
                logger( $_SESSION['email']." authenticated with cookie {$_COOKIE['rememberme']}: "."\"display\":"."\"".$line['display_name']."\""." \"nicename\":"."\"".$line['user_nicename']."\""." \"priority\":".$line['priority']." \"admin\":".$line['admin'], 0, "auth", "db", "get_user");
	        }
            else
    ...
    $userJSON .= "}";
    $userJSON .= "}";
    //logger($userJSON, 0, "auth", "db", "get_user");
    echo $userJSON;
     ...
     $dbarray = get_user_from_email($email);
      ...
      if($password == $dbarray['u_pswd']){
	    ...
	    if ($locked_value)
			...
			if ($isrememberme){
                logger("confirmUserPass: ".$_SESSION['email']." setting cookie rememberme = ".$dbarray['user_rememberme'], 0, "auth", "db", "confirmUserPass");

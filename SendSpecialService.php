...
function sendAPNToRegIDs($registrationIDs, $message, $picture_url, $embedded_url){
    ...
    $resultAPNs .= ' APNs Completed';

    @socket_close($apns);
    fclose($apns);
    return $errorString." ".$resultAPNs;
}
function sendAPNMessage($messageBody, $title, $picture_url, $embedded_url)
{
...
//$reg_id = "d057506a9d09770900a09fbeb25c9e404829937bc1b3da0d34216f9cc57608e5";
...
function sendGCMMessage($messageBody, $title, $picture_url, $embedded_url)
{
...
function callGCMSender($registrationIDs, $messageBody, $title, $picture_url, $embedded_url){
    ...
    switch ($resultHttpCode) {
            ...
            default:
                //TODO: Retry-after
                throw new Exception("Unknown error. ".$result, Exception::UNKNOWN_ERROR);
                break;
        }
    return $result;
}
...
if (empty($empty)) {
   ...
   //$result .= "Empty:\n";  var_dump($empty);
    ...
    try{
    updateMessageFromMessages ($message[0]."<br />".$title[0].". ".$img_tag, 1, 'forecast', 0 ,'' ,'','');
    updateMessageFromMessages ($message[1]."<br />".$title[1].". ".$img_tag, 1, 'forecast', 1 ,'' ,'','');
    } catch (Exception $ex) {
        $result .= " exception updateMessageFromMessages:".$ex->getMessage();
    }
    try{
        sendGCMMessage($msgSpecial, $title, $picture_url, $embedded_url);   
    } catch (Exception $ex) {

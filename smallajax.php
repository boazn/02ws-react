...
<div id="latesttemp" class="inparamdiv" style="display:none;">
        ...
        <div class="paramvalue">
         ...
         <div class="highlows">
                 <div class="high"><strong></strong></div>&nbsp;<img src="img/peak_max.png" width="15" height="14" alt="<? echo $HIGH[$lang_idx]; ?>"/>&nbsp;<span class="high_time"></span>
                 <div class="low"><strong></strong></div>&nbsp;<img src="img/peak_min.png" width="15" height="14" alt="<? echo $LOW[$lang_idx]; ?>"/>&nbsp;<span class="low_time"></span>
         </div> 
...
<div id="latesthumidity" class="inparamdiv" <? if (isHeb()) echo "dir=\"rtl\" ";?> style="display:none">
    <div class="paramtitle slogan">
        ...
        <div class="highlows">
            <div class="highparam"><strong></strong></div>&nbsp;<img src="img/peak_max.png" width="15" height="14" alt="<? echo $HIGH[$lang_idx]; ?>"/>&nbsp;<span class="high_time"></span>
            &nbsp;&nbsp;&nbsp;&nbsp;<div class="lowparam"><strong></strong></div>&nbsp;<img src="img/peak_min.png" width="15" height="14" alt="<? echo $LOW[$lang_idx]; ?>"/>&nbsp;<span class="low_time"></span>
         </div>
       <div class="paramtrend relative">
            <div class="innertrendvalue">
...
<div id="livepic_box" class="white_box" style="display:none">
	...
	<h3><a id="forum" href="<?=$_SERVER['SCRIPT_NAME']?>?section=chat.php&amp;lang=<?=$lang_idx;?>">
    ...
    function fillAllJson(jsonstr)
    {
             ...
             try{
              ...
              try{
                ...
                var cssstyle_str = "";
                 if (json.jws.current.issunset == 'true') { 
					cssstyle_str += "sunset"; 
				    loadCSS("css/sunset.min.css");
					}
                if (json.jws.current.issunrise == 'true') { 
					cssstyle_str += "sunrise"; 
					loadCSS("css/sunset.min.css");
				} 
                if ((json.jws.current.islight)&&(json.jws.current.cloudiness > 6)) { 
					cssstyle_str += ",cloudy"; 
					loadCSS("css/cloudy.min.css");
				}
                if (json.jws.current.pm10 > 250) { 
					cssstyle_str += ",dust"; 
					loadCSS("css/dust.min.css");
				} 
                if (!json.jws.current.islight) { 
					cssstyle_str += ",night";
					loadCSS("css/night.min.css"); 
                    if (json.jws.current.pm10 > 250) { cssstyle_str += ",dust-night"; }
                }
               if (json.jws.states.israining == 'true'){ 
				   cssstyle_str += ",rain"; 
				   loadCSS("css/rain.min.css");
			   }
               if (json.jws.states.issnowing == 'true') { 
                     if (json.jws.current.islight){ 
						 $cssstyle_str += ",snow"; 
						 loadCSS("css/snow.min.css");
                    } else { 
						cssstyle_str += ",snow_night"; 
						loadCSS("css/snow_night.min.css");
					} 
               }
               cssstyle_str += ",mobile";
               
			     $('#date').html(json.jws.current.date<?=$lang_idx?>); 
                 $('#logotitle').html(json.jws.logo.logo<?=$lang_idx?>); 
                 ...
                 {$("#itfeels_windchill").show();$("#itfeels_windchill .value").html(json.jws.feelslike.value + "&#176;")}
                 else if (json.jws.feelslike.state == "heatindex")
                 {$("#itfeels_heatidx").show();$("#itfeels_heatidx .value").html(json.jws.feelslike.value + "&#176;")}


			     $("#latesttemp .paramvalue").html(json.jws.current.temp+'<span class="paramunit">'+json.jws.current.tempunit+'</span>');
                 $("#latesttemp .highlows .high").html('<strong>' + json.jws.today.hightemp + '</strong>');
                 $("#latesttemp .highlows .high_time").html(json.jws.today.hightemp_time);
                 $("#latesttemp .highlows .low").html('<strong>' + json.jws.today.lowtemp + '</strong>');
                 ...
                 for (i = 0; i< 4; i++){ 
                 $('#shortforecast').children('.nav').children().eq(i).html(json.jws.forecastDays[i].day_name<?=$lang_idx?>+"  "+json.jws.forecastDays[i].date+"</br>"+"<img src=\"" + json.jws.forecastDays[i].icon + "\" width=\"32\" height=\"32\" alt=\"" + json.jws.forecastDays[i].icon +"\" /></br>"+json.jws.forecastDays[i].TempLow+"-"+json.jws.forecastDays[i].TempHigh);
                 }
                 var forecastHours = "";
                 ...
                 if ((json.jws.forecastHours[i].time % 3 == 0) || (json.jws.forecastHours[i].plusminus > 0))
                 {
                    ...
                    forecastHours += "<li class=\"tsfh\" style=\"display:none\"><span>" + json.jws.forecastHours[i].currentDateTime + "</span></li>";
                    ...
                    forecastHours += "<li style=\"margin-top:0;\"><div title=\"" + json.jws.forecastHours[i].wind_title+"\" class=\"wind_icon "+json.jws.forecastHours[i].wind_class+" \"></div></li>";
                    ...
                    forecastHours += "<li class=\"forcast_title\"><span>"+json.jws.forecastHours[i].title+"</span></li>";
                    forecastHours += "</ul>";
                 }
                ...
                forecastDays = "<table style=\"border-spacing:4px;width:100%\">";
                 ...
                 forecastDays += "<tr style=\"height:" + 180/json.jws.forecastDays.length + "px\">";
                 ...
                 forecastDays += "<td class=\"tsfh\" style=\"text-align:center;direction:ltr\">" + json.jws.forecastDays[i].TempLow +"</td>";
                 forecastDays += "<td class=\"tsfh\" style=\"text-align:center;direction:ltr\">" + json.jws.forecastDays[i].TempHigh + '<a href=\"WhatToWear.php#\" rel=\"external\" ><img style=\"vertical-align: middle\" src=\"'+json.jws.forecastDays[i].TempHighCloth+'\" width=\"20\" height=\"15\" title=\"'+json.jws.forecastDays[i].TempHighClothTitle+'\" alt=\"\" /></a>' + "</td>";
                ...
                $('#messages_box').children('.box_text').html(json.jws.Messages.detailedforecast<? echo $lang_idx;?>); 
                ...
                $('#livepic_box').show();
				$('#canvas').show();
                if ((json.jws.states.issnowing != 'true') && (json.jws.states.israining == 'true'))    
                {document.write("<script type=\"text/javascript\" src=\"js/rain.js\"></script>")}
                else if (json.jws.states.issnowing == 'true')    

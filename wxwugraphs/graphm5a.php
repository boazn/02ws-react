...
<html>
	<head>
		...
		<title>'.$WSTran.' '.$mnthNameYear.' - '.$gSubtitle.'</title>
...
echo $hchExport;
echo '			   title: {
			      text: "'. $WSTran.$TperMonth.$mnthNameYear.'"
			   },
			   credits: {
			   ...
			   tooltip: {
                 ...
                 var degw = '';
                      ...
                      var degw = wword+' - ';
  			                ...
  			                return '<b>'+ this.series.name +'<\/b><br\/><span style="font-size:12pt;">'+ degw + this.y +'<\/span><b>'+windunits+'<\/b>'+'<br\/>'+ Highcharts.dateFormat('<?php echo $ttDateText[$ddFormat]; ?>', this.x);
			      }
			   },
        colors: [ '#B5CA92', '#6DEB5A', '#A5EF26', '#4572A7', '#AA4643', '#80699B',  '#DB843D', '#92A8CD', '#A47D7C' ],
			   plotOptions: {
			      <?php echo $aspline ?>: {	         
			         marker: {
			           enabled: false,

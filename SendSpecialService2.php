...
function sendAPNMessage($messageBody, $title, $picture_url, $embedded_url, $short_range, $long_range, $tip, $dailyforecast)
{
   ...
   $query_extension = "";
    ...
    if ((boolval($long_range))&&(boolval($short_range))){
        ...
        $messageBody = getFirstForecastDay();
...
function sendGCMMessage($messageBody, $title, $picture_url, $embedded_url, $short_range, $long_range, $tip, $CloudMessageType, $dailyforecast)
{
...
function handleInvalidTokens($jsonArray, $registration_ids, $header_key){
    ...
    foreach ($remove_ids as $id){
     ...
     foreach ($errorMessage as $err){
         //print_r($regIDs);
         $query = "update gcm_users set ResponseMessage=".$err['desc']." where gcm_regid='".$err['id']."'";
         $resultUpdate = mysqli_query($link, $query);
...
function callGCMSender($key, $registrationIDs, $messageBody, $title, $picture_url, $embedded_url){
    ...
    logger("callGCMSender: header_key(SenderID)=".$key."  count=".count($registrationIDs)." message=".$messageBody." title=".$title." picture_url=".$picture_url." embedded_url=".$embedded_url);
    // Set POST variables
...
if (empty($empty)) {
   ...
   $result .= "Empty:\n";  var_dump($empty);
    ...
    try {
        ...
        //$result .= send_Email(array($_REQUEST['message0']." ".$img_tag, $_REQUEST['message1']." ".$img_tag), ALL, EMAIL_ADDRESS, "", "", $EmailSubject);
    } 
    catch (Exception $ex) {
